...
function [SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,...
	  betaBASIS_PHI,beta] = ...
    FullStatistics(LIKELIHOOD,BASIS,PHI,Targets,Used,Alpha,beta,...
		       Mu,BASIS_PHI,BASIS_Targets,OPTIONS)
...
if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian
  ...
  for i = 1:size(PHI,2)
       for j = 1:size(PHI,2)
       PHI_2(i,j) = sum(bsxfun(@times,PHI(:,i),PHI(:,j)));
       end
  end
  for i = 1:size(Alpha,2)
      for j = 1:size(Alpha,2)
       Alpha_2(i,j) = sum(bsxfun(@times,Alpha(:,i),Alpha(:,j)));
      end
  end
  S1 = size(PHI_2)
  S2  = size(Alpha_diag)
  
  size(PHI_2*beta + Alpha_2)
  U		= chol(PHI_2*beta + Alpha_2);
  Ui	= inv(U);
  SIGMA	= Ui * Ui';
  % Posterior mean Mu
