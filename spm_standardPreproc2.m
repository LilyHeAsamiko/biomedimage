function output = spm_standardPreproc(functional4D_fn, structural_fn, fwhm, spm_dir)
%From SPM12 tutorial
%
% INPUT:
...
%           'none'    - no regularisation________________________________________________________________________
%Stephan Heunis 2018
%LilyHe 

%spm_defaults('fmri')
% Load data
%f_fn = 'P:\biomedical imaging\ds000157_R1.0.3\ds000157_R1.0.3\sub-1\anat\sub-1_T1w.nii'
functional4D_fn = f_fn;
...
% Roptions
realign_estimate_reslice.matlabbatch{1}.spm.spatial.realign.estwrite.roptions.which = [2 1];% all images and mean
...
realign_estimate_reslice.matlabbatch{2} = realign_estimate_reslice.matlabbatch{1};
% Run
...
coreg_estimate.matlabbatch{2} = coreg_estimate.matlabbatch{1};
% Run
...
% Warp
segmentation.matlabbatch{1}.spm.spatial.preproc.warp.mrf = 1;% minimum residual flow
...
segmentation.matlabbatch{2} = segmentation.matlabbatch{1};
% Run
...
reslice.matlabbatch{2} = reslice.matlabbatch{1};
% Run
...
smooth.matlabbatch{2} = smooth.matlabbatch{1};
% Run
