...
function [PARAMETER, HYPERPARAMETER, DIAGNOSTIC] = ...
    SparseBayes( BASIS, Targets, SETTINGS,  OPTIONS)
...
% 
%OPTIONS = Diagnostic(OPTIONS, 'start');
...
while (~LAST_ITERATION)
  ...
  if CONTROLS.BasisAlignmentTest
    ...
    if selectedAction==ACTION_DELETE
      ...
      if numAligned>0
		...
		Diagnostic(OPTIONS,3,'Alignment reinstatement of %s\n', r_);
      end
    end
  end
  ...
  M		= length(Used);
  %
  Diagnostic(OPTIONS, 3, 'ACTION: %s of %d (%g)\n', ...
  ...
  if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian && ...
    ...
    if abs(deltaLogBeta) > CONTROLS.MinDeltaLogBeta
      ...
      if selectedAction==ACTION_TERMINATE
		...
		Diagnostic(OPTIONS,3,'Noise update (termination deferred)\n');
      end
    end
  end
  ...
  if (selectedAction==ACTION_TERMINATE)
    ...
    Diagnostic(OPTIONS,2,...
				   ...
				   i, deltaLogMarginal);
    if LIKELIHOOD.InUse~=LIKELIHOOD.Gaussian
      Diagnostic(OPTIONS,2,'%4d> L = %.6f\t Gamma = %.2f (M = %d)\n',...
  ...
  if (OPTIONS.monitor && ~rem(i,OPTIONS.monitor)) || LAST_ITERATION
	...
	% 
    if LIKELIHOOD.InUse~=LIKELIHOOD.Gaussian
      Diagnostic(OPTIONS,2,'%5d> L = %.6f\t Gamma = %.2f (M = %d)\n',...
		     i, logML/N, sum(Gamma), M);
    else
      Diagnostic(OPTIONS,2,...
...
if OPTIONS.diagnosticLevel>1
  ...
  %
  Diagnostic(OPTIONS,2,'Action Summary\n');
  Diagnostic(OPTIONS,2,'==============\n');
  Diagnostic(OPTIONS,2,'Added\t\t%6d (%.0f%%)\n',...
		 addCount, 100*addCount/total);
  Diagnostic(OPTIONS,2,'Deleted\t\t%6d (%.0f%%)\n',...
		 deleteCount, 100*deleteCount/total);
  Diagnostic(OPTIONS,2,'Reestimated\t%6d (%.0f%%)\n',...
		 updateCount, 100*updateCount/total);
  %
  if CONTROLS.BasisAlignmentTest && alignDeferCount
    Diagnostic(OPTIONS,2,'--------------\n');
    Diagnostic(OPTIONS,2,'Deferred\t%6d (%.0f%%)\n',...
		   alignDeferCount, 100*alignDeferCount/total);
  end
  %
  Diagnostic(OPTIONS,2,'==============\n');
  Diagnostic(OPTIONS,2,'Total of %d likelihood updates\n', count);
  Diagnostic(OPTIONS,2,'Time to run: %s\n', SB2_FormatTime(t1));
end

% Terminate diagnostics
OPTIONS = Diagnostic(OPTIONS, 'end');
